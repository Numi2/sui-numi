...
impl AdmissionControl {
	pub fn new(max_inflight: usize, rate_per_sec: Option<u32>) -> Self {
		...
		Self { max_inflight: Arc::new(Semaphore::new(max_inflight)), inner: Arc::new(Mutex::new(rl)) }
	}
	...
	pub async fn acquire(&self) -> AdmissionPermit {
		...
		let permit = self.max_inflight.clone().acquire_owned().await.expect("semaphore not closed");
...
impl CircuitBreakers {
	...
	pub async fn is_open(&self, class: &str) -> bool {
		...
		let b = inner.entry(class.to_string()).or_insert_with(Breaker::default);
